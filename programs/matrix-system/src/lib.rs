...
security_txt! {
    ...
    policy: "https://github.com/ghost-ai91/matrix/blob/main/SECURITY.md",
    preferred_languages: "en",
    source_code: "https://github.com/ghost-ai91/matrix/blob/main/programs/matrix-system/src/lib.rs",
...
// New structure to store complete information for each upline
...
// Modified referral upline structure
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Default)]
pub struct ReferralUpline {
    pub id: u32,
    pub depth: u8,
    pub upline: Vec<UplineEntry>, // Modified: now stores UplineEntry with all information
}
...
// Event structures - Keeping only SlotFilled
#[event]
...
) -> Result<(i128, u32, i64, i64)> { // ADICIONADO: retorna também o feed_timestamp
...
// Function to calculate minimum SOL deposit based on USD price - MODIFICADO
...
pub struct RegisterWithoutReferrerDeposit<'info> {
    ...
    /// CHECK: User token account for source token (Wrapped SOL)
    #[account(mut)]
...
pub mod referral_system {
    ...
    pub fn register_without_referrer(ctx: Context<RegisterWithoutReferrerDeposit>, deposit_amount: u64) -> Result<()> {
        ...
        // Deposit to liquidity pool (same logic as slot 1)
